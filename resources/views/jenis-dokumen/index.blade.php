<x-layouts.app>
    <div class="p-6">
        ...
        <x-data-table :data="$jenisDokumens" paginated>
            ...
            <x-slot name="header">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Master data jenis dokumen / sertifikat kapal
                    </p>
                </div>
            </x-slot>

            <x-slot name="search">
                <form method="GET" action="{{ route('jenis-dokumen.index') }}" class="flex gap-2">
            ...
            <x-slot name="columns">
                <x-table-column>Nama Dokumen</x-table-column>
                <x-table-column>Masa Berlaku (Bulan)</x-table-column>
                <x-table-column>Aksi</x-table-column>
            </x-slot>

            <x-slot name="rows">
                @forelse ($jenisDokumens as $j)
                    <x-table-row>

                        <x-table-cell>{{ $j->nama }}</x-table-cell>
                        <x-table-cell>{{ $j->masa_berlaku }}</x-table-cell>

                        <x-table-cell>
                            <div class="flex gap-1">
                                <a href="{{ route('jenis-dokumen.edit', $j->id) }}"
                                    class="text-amber-500 hover:text-amber-700 dark:text-amber-400 dark:hover:text-amber-300 p-1.5 rounded transition"
                                    title="Edit">
                                    <i class="fa-solid fa-pen-to-square text-lg"></i>
                                </a>

                                <form action="{{ route('jenis-dokumen.destroy', $j->id) }}" method="POST"
