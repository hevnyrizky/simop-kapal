<x-layouts.app>
    <div class="p-6">
        ...
        <x-data-table :data="$klasifikasis" paginated>
            ...
            <x-slot name="header">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Master data badan klasifikasi kapal
                    </p>
                </div>
            </x-slot>

            <x-slot name="search">
                <form method="GET" action="{{ route('klasifikasi.index') }}" class="flex gap-2">

                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama / negara / keterangan..."
            ...
            <x-slot name="columns">
                ...
                <x-table-column>Negara</x-table-column>
                <x-table-column>Keterangan</x-table-column>
                <x-table-column>Aksi</x-table-column>
            </x-slot>

            <x-slot name="rows">
                @forelse ($klasifikasis as $k)
                    <x-table-row>

                        <x-table-cell>{{ $k->nama }}</x-table-cell>
                        <x-table-cell>{{ $k->negara ?: '-' }}</x-table-cell>
                        <x-table-cell>{{ $k->keterangan ?: '-' }}</x-table-cell>

                        <x-table-cell>
                            <div class="flex gap-1">
                                <a href="{{ route('klasifikasi.edit', $k->id) }}"
